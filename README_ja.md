...
- 入力フォルダ以下のCSVを再帰的に収集（フォルダ構造・階層数自由）
...
2列（`y, x`）または3列（`time, y, x`）のCSVファイル（ヘッダなし）。2列の場合は連番のtimeを自動生成する。y列またはx列に数値以外を含む行（`START`/`END`等の文字列行）は自動除去される。ペンオフ（`0,0`またはNaN）は欠損として処理され、先頭・末尾は除去、中間は補間される。

ファイル名形式: `<被験者ID>_<条件名>.csv`（例: `A_CDL.csv`）

フォルダ構造は自由。入力フォルダ以下にあるCSVファイルを階層・構造に関わらず全て収集する。被験者ごとにフォルダを分けても、全てフラットに並べても、混在させても動作する。id・conditionはフォルダ名ではなくファイル名のみから判定される。ファイル名に`_`が含まれない場合、idは`unknown`として扱われる。
...
- CSV内の全数値行を処理対象とするため、特定のマーカー文字列で計測範囲を指定することはできない。記録開始から終了までの全区間が解析対象となる前提が必要
...
- 指定文字列による有効範囲の限定オプション（例：`range_start="START"`, `range_end="END"` のように指定した文字列行の間のみを処理対象にする）
