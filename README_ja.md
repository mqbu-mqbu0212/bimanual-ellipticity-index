...
PCAは回転不変のため、軸合わせ補正は不要。真円に近いサイクル（flatness ≥ 0.85）に対しては、始点と終点の中点を求め、どちらの主軸により近いかを判定することで軸の曖昧さを解消する。この軸判定では、軸の割り当てによってOI > 1.0となることがある。デフォルト（`clip_oi_to_one=True`）ではこれらの値を1.0にクリップする。`clip_oi_to_one=False`にするとそのまま保持される。
...
人間が手で描く軌跡はサイクルごとに中心座標がずれるのが避けられず、tail部分はこのずれの過程を含むことが多い。このずれは定量化できないため、OI計算に含めると結果が歪む。trimは恣意的な介入ではなく、被験者が意図した楕円部分のみを抽出するための設計上の選択である。実データではtrimによってOIが0.7892から0.8417に変化した例がある。
...
人間が手で描く軌跡はサイクルごとにずれが生じるため、tailは意図した楕円ではなくそのずれの過程を含むことが多い。tailが閾円に入り脱出後も前半軌跡からr未満の距離を保つ場合、tail内の始点最近傍点でtrimする。以下はtrim前後の例（OI: 0.7892 → 0.8417）。
...
- OI計算オプションの追加（PCA主軸基準 / y軸基準 / Q軸近傍基準）
- ルールベースの楕円フィッティングフィルタをMLによるサイクル品質分類に置き換え（Random Forest / LSTM / CNN）
