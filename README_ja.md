...
**結果:** 明示的・調整可能なパラメータによる再現性のあるOI値。両手のデータを対象に任意の両手協調データセットへ適用可能。
...
人間が手で描く軌跡はサイクルごとにずれが生じるため、tailは意図した楕円ではなくそのずれの過程を含むことが多い。tailが閾円に入り脱出後も前半軌跡からr未満の距離を保つ場合、tail内の始点最近傍点でtrimする。

余分なtailが含まれると、軌跡全体の重心がずれ、PCAで求めるsd_majorとsd_minorの比が本来の楕円形状を反映しなくなる。trimすることで意図した楕円部分のみの分布に基づいたOIが得られる。以下はtrim前後の例（OI: 0.7892 → 0.8417）。
